...
- **Контексты**: React Context API для глобального управления состоянием приложения, которое должно быть доступно многим компонентам (например, текущие настройки, тема, язык) в `frontend/src/contexts`
- **Пользовательские хуки**: Переиспользуемая логика компонентов для локального управления состоянием и побочными эффектами (например, useWindowSize, useDebounce, useTorrentSort) в `frontend/src/hooks`
