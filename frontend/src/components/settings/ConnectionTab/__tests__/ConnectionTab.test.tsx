...
describe('ConnectionTab Component', () => {
    ...
    // Новый тест для покрытия ветки ?? ""
    it('renders port input with empty string when port is undefined', () => {
        renderComponent({ settings: { ...defaultSettings, port: undefined } });
        expect(screen.getByTestId('connection-port-input')).toHaveValue(null); // В input type="number" пустое значение соответствует null
    });
